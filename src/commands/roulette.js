	...
	options: [
		{
			name: "type",
			description:
				"all bets have a 1-1 payout ratio, except green that has a 35-1 because you bet on a single number",
			required: true,
			type: "STRING",
			choices: [
			...
			if (
				...
				if ((await readFile(user.id, "falcoins")) >= bet && bet > 0) {
					...
					const embed = new MessageEmbed()
						...
						.setDescription(
							instance.messageHandler.get(guild, "GIRANDO_ROLETA")
						)
					...
					if (type.includes(luck)) {
							...
							.addFields(
								{
									name:
										...
										" :sunglasses:",
									value:
										instance.messageHandler.get(guild, "BOT_ROLOU") +
										` **${luck}**`,
									inline: true,
								},
								{
						...
						var embed2 = new MessageEmbed()
							...
							.addFields(
								{
									name:
										instance.messageHandler.get(guild, "VOCE_PERDEU") +
										" :pensive:",
									value:
										instance.messageHandler.get(guild, "BOT_ROLOU") +
										` **${luck}**`,
